...
with gr.Blocks(theme=gr.themes.Base(), title='TTS your model') as app:
    with gr.Tabs():
        with gr.TabItem("Интерфейс"):
            gr.HTML("<h1>ГЕНЕРАЦИЯ TTS С ВАШЕЙ МОДЕЛЬЮ</h1>")
            ...
            # Other RVC stuff
            with gr.Row():
                sid0 = gr.Dropdown(label="Выберите свою модель", choices=sorted(names), value=check_for_name())
                ...
                but0 = gr.Button("Сгенерировать", variant="primary")
            with gr.Row():
                with gr.Column():
                    with gr.Row():
                        input_audio0 = gr.Dropdown(
                            label="Выберите свое аудио",
                    ...
                    with gr.Row():
                        with gr.Accordion('Текст в Речь'):
                            with gr.Column():
                            ...
                            with gr.Column():
                                ...
                                tts_button = gr.Button(value="Сгенерировать")
                                tts_button.click(fn=elevenTTS, inputs=[api_box,tfs, elevenid, lang], outputs=[record_button, input_audio0])

                with gr.Column():
                    with gr.Accordion("Настройка индекса", open=False):
                        file_index1 = gr.Dropdown(
                            label="Путь к файлу added.index (если он не был найден автоматически).",
                            choices=get_indexes(),
                            value=get_index(),
                            interactive=True,
                            )
                    ...
                    with gr.Accordion("Дополнительные настройки", open=False):
                        f0method0 = gr.Radio(
                            label="Необязательно: Изменить алгоритм извлечения питча. Методы извлечения сортируются от 'худшего качества' к 'лучшему качеству'. mangio-crepe может быть лучше rmvpe или нет в случаях, когда 'гладкость' более важна, но в целом rmvpe является лучшим.",
        ...
        with gr.TabItem("Download Model"):
            with gr.Row():
                url=gr.Textbox(label="Введите URL адрес вашей модели:")
            with gr.Row():
                model = gr.Textbox(label="Имя вашей модели:")
